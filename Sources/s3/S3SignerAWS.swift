...
public class S3SignerAWS  {
	...
	public func authHeaderV4(httpMethod: HTTPMethod, urlString: String, pathPercentEncoding: CharacterSet =  CharacterSet.urlPathAllowed, queryPercentEncoding: CharacterSet = CharacterSet.urlQueryAllowed, headers: [String: String], payload: Payload, mimeType: String? = nil, accessControl: AccessControlList = .privateAccess) throws -> [String:String] {
        
        guard let url = URL(string: urlString) else { throw S3SignerError.badURL }
        
        let dates = getDates(date: Date())
        
        let bodyDigest = payload.hashed()
        ...
        if httpMethod == .put && payload.isBytes{
            updatedHeaders["content-md5"] = payload.bytes.md5().toBase64()
        }
		
		updatedHeaders["x-amz-acl"] = accessControl.rawValue
        ...
        if httpMethod == .put {
            let payloadSize = payload.isBytes ? payload.bytes.count.description : payload.hashed()
        ...
        if payload.isUnsigned {
            updatedHeaders["x-amz-content-sha256"] = payload.hashed()
        }
    ...
    fileprivate func getSignature(stringToSign: String, timeStampShort: String) throws -> String {
		let key = Array("AWS4\(secretKey)".utf8)
		let content = Array(timeStampShort.utf8)
        let dateKey = try HMAC(key: key, variant: .sha256).authenticate(content)
        let dateRegionKey = try HMAC(key: dateKey, variant: .sha256).authenticate(Array(region.rawValue.utf8))
        let dateRegionServiceKey = try HMAC(key: dateRegionKey, variant: .sha256).authenticate(Array("s3".utf8))
        let signingKey = try HMAC(key: dateRegionServiceKey, variant: .sha256).authenticate(Array("aws4_request".utf8))
        let signature = try HMAC(key: signingKey, variant: .sha256).authenticate(Array(stringToSign.utf8)).toHexString()
        return signature
    }
	...
	fileprivate func canonicalHeaders(headers: [String: String]) -> String {
		let headerList = Array(headers.keys).sorted {
